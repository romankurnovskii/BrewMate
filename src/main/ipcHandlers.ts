  ...
  ipcMain.on('get-all-apps', async (event: IpcMainEvent) => {
    ...
    try {
      ...
      try {
        ...
        // Optimization: Pre-allocate array and use native for loops to avoid
        // intermediate array allocations and GC overhead on massive datasets (~100k items).
        const allApps = new Array<App>(casks.length + formulas.length);
        let appIndex = 0;
