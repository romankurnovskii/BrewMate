  ...
  ipcMain.on('get-all-apps', async (event: IpcMainEvent) => {
    ...
    try {
      ...
      try {
        ...
        // Optimization: When combining large datasets (~100k items), avoid using spread syntax (...) with .map()
        // as it creates multiple intermediate arrays causing high memory allocations and Garbage Collection (GC) overhead.
        // Instead, use a pre-allocated array and populate it via native for loops.
        const totalApps = casks.length + formulas.length;
        const allApps: App[] = new Array(totalApps);
        let appIndex = 0;
