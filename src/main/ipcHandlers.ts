  ...
  ipcMain.on('get-all-apps', async (event: IpcMainEvent) => {
    ...
    try {
      ...
      try {
        ...
        // Optimization: Pre-allocate the array and use native for loops instead of spread syntax and .map()
        // to prevent large intermediate array allocations and reduce GC overhead when merging ~100k items.
