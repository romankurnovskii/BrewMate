  ...
  ipcMain.on('get-all-apps', async (event: IpcMainEvent) => {
    ...
    try {
      ...
      try {
        ...
        // Optimization: When combining large datasets (~150k items), avoid using spread syntax (...)
        // with .map() as it creates multiple intermediate arrays causing high memory allocations and GC overhead.
