  ...
  ipcMain.on('get-all-apps', async (event: IpcMainEvent) => {
    ...
    try {
      ...
      try {
        ...
        // Optimization: Use a pre-allocated array and native loops rather than spread syntax (...) and .map()
        // to prevent the main thread from blocking due to massive garbage collection (GC) overhead when processing ~150k items.
