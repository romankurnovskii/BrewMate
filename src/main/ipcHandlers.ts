  ...
  ipcMain.on('get-all-apps', async (event: IpcMainEvent) => {
    ...
    try {
      ...
      try {
        ...
        // Optimization: Use pre-allocated array and native for loops to avoid multiple
        // intermediate array allocations and GC overhead when processing ~100k items.
        const allApps: App[] = new Array(casks.length + formulas.length);
        let index = 0;
