  ...
  ipcMain.on('get-all-apps', async (event: IpcMainEvent) => {
    ...
    try {
      ...
      try {
        ...
        // Optimization: Use a pre-allocated array and native for loops instead of spread syntax and .map()
        // This avoids creating multiple intermediate arrays and reduces GC overhead for large datasets (~100k items).
