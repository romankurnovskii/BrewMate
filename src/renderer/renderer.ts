...
function setupEventListeners(): void {
  ...
  ipcRenderer.on('all-apps', (_event: any, apps: Array<App>) => {
    ...
    // Optimization: Build a Map of all apps for O(1) lookups by name instead of O(N) array scanning
    allAppsMap.clear();
...
function updateDashboardView(): void {
  ...
  for (const appName of installedApps) {
    // Optimization: Use O(1) Map lookup instead of O(N) Array.find() inside a loop,
    // reducing complexity from O(N * M) to O(M) where N=~100k all apps and M=installed apps
    const app = allAppsMap.get(appName);
    if (app) {
      if (app.type === 'cask') {
        caskCount++;
