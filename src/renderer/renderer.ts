...
function renderCategories(): void {
  ...
  while (categoryChips.firstChild) {
    categoryChips.removeChild(categoryChips.firstChild);
  }
...
function filterApps(): void {
  ...
  requestAnimationFrame(() => {
    ...
    // Pre-calculate search term in lower case once outside the loop
    const searchLower = (searchTerm || '').toLowerCase();
    const hasSearch = searchLower.length > 0;

    filteredApps = allApps.filter((app) => {
      // 1. Fastest: Direct property check
      if (selectedType !== 'All' && app.type !== selectedType) {
        return false;
      }

      // 2. Fast: Set lookup or cached property check
      if (selectedCategory === 'Installed') {
        if (!installedApps.has(app.name)) return false;
      } else if (selectedCategory !== 'All') {
        // Memoize category on the app object to prevent recalculating string operations
        if (!app._categoryCache) {
          app._categoryCache = getCategoryForApp(app);
        }
        if (app._categoryCache !== selectedCategory) return false;
      }

      // 3. Slowest: String operations and searching
      if (hasSearch) {
        const name = (app.name || '').toLowerCase();
...
function renderApps(): void {
  ...
  visibleApps.forEach((app) => {
    ...
    // renderAppCard could be refactored to return an HTMLElement, but for now we'll
    // parse the HTML string it returns. Note: app data is escaped via escapeHtml.
    // However, to be completely safe, we shouldn't use innerHTML.
    // We will use DOMParser.
    const parser = new DOMParser();
    const doc = parser.parseFromString(renderAppCard(app, isInstalled), 'text/html');
    const appCard = doc.body.firstChild as HTMLElement;
    if (appCard) {
  ...
  appsGrid.replaceChildren(outerDiv);
