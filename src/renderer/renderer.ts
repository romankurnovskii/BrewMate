...
function filterApps(): void {
  ...
  requestAnimationFrame(() => {
    ...
    if (
      ...
      // Pre-compute invariant search term outside the loop
      ...
      filteredApps = allApps.filter((app) => {
        ...
        // 2. Set lookup and property comparisons
        if (selectedCategory === 'Installed') {
          if (!installedApps.has(app.name)) return false;
        } else if (selectedCategory !== 'All') {
          // Memoize the calculated category on the item object to prevent UI thread blocking
          if (!app._category) {
