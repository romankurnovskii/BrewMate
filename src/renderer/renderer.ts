...
function filterApps(): void {
  ...
  requestAnimationFrame(() => {
    ...
    // ⚡ Bolt: Cache search term lowercase conversion outside the filter loop
    // Impact: Avoids redundant string allocations and `.toLowerCase()` calls for every app (O(N) operation)
    // Measurement: Faster responsiveness when filtering through the large list of available apps
