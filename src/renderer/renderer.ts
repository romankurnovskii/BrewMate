...
function renderCategories(): void {
  categoryChips.textContent = ''; // Clear existing chips safely
...
function filterApps(): void {
  ...
  requestAnimationFrame(() => {
    ...
    // Hoist variables outside the loop to improve performance
    ...
    filteredApps = allApps.filter((app) => {
      if (!isTypeAll && app.type !== selectedType) {
        return false;
      }
      ...
      if (!isCategoryAll) {
        if (isCategoryInstalled) {
          ...
          // Lazy evaluation and caching of category to prevent repeated regex/string operations
          if (app._category === undefined) {
            app._category = getCategoryForApp(app);
          }
          if (app._category !== selectedCategory) {
            return false;
          }
        }
      }
      ...
      if (desc && desc.toLowerCase().includes(searchLower)) return true;

      const homepage = app.homepage;
      if (homepage && homepage.toLowerCase().includes(searchLower)) return true;
