...
function setupEventListeners(): void {
  ...
  ipcRenderer.on('all-apps', (_event: any, apps: Array<App>) => {
    ...
    // Pre-calculate lowercased fields for faster filtering
    apps.forEach((app) => {
...
function filterApps(): void {
  ...
  requestAnimationFrame(() => {
    // Pre-calculate search string outside the loop
    const searchLower = searchTerm ? searchTerm.toLowerCase() : '';

    filteredApps = allApps.filter((app) => {
      // Early return: check type first (fastest)
      if (selectedType !== 'All' && app.type !== selectedType) {
        return false;
      }

      // Early return: check category next
      ...
      // Early return: check search last (slowest, but uses cached lower strings)
      if (searchLower) {
        const nameLower =
