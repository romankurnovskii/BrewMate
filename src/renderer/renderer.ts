...
function filterApps(): void {
  ...
  requestAnimationFrame(() => {
    ...
    // Pre-calculate invariants outside the loop for O(n) performance
    ...
    filteredApps = allApps.filter((app) => {
      ...
      if (isTypeFilterActive && app.type !== selectedType) {
        return false;
      }

      // 2. Medium: Set lookup / String matching
      ...
      // 3. Slowest: Multiple string includes (only checked if others pass)
      if (hasSearch) {
        const name = (app.name || '').toLowerCase();
