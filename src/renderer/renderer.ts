...
function setupEventListeners(): void {
  ...
  ipcRenderer.on('all-apps', (_event: any, apps: Array<App>) => {
    ...
    // Bolt: Pre-compute derived properties to drastically speed up filtering
    ...
    (_event: any, { loading, message }: { loading: boolean; message?: string }) => {
      ...
      if (loading) {
        if (!appsGrid.querySelector('.loading')) {
          ...
          loadingMsg.textContent = message || 'Loading apps...';
...
function filterApps(): void {
  ...
  requestAnimationFrame(() => {
    // Bolt: Move invariant transformations outside the loop
    const searchLower = (searchTerm || '').toLowerCase();

    filteredApps = allApps.filter((app) => {
      // Early return based on type (fastest check)
      if (selectedType !== 'All' && app.type !== selectedType) {
        return false;
      }

      // Early return based on category
      if (selectedCategory === 'Installed') {
        if (!installedApps.has(app.name)) return false;
      } else if (selectedCategory !== 'All') {
        // Bolt: Use memoized category if available
        const appCategory = app._category || getCategoryForApp(app);
        if (appCategory !== selectedCategory) return false;
      }

      // Early return based on search term
      if (searchLower) {
        // Bolt: Use memoized lowercased strings
        const name =
        ...
        const homepage =
          app._homepageLower !== undefined
            ? app._homepageLower
            : (app.homepage || '').toLowerCase();
...
function renderApps(): void {
  ...
  if (isLoading && allApps.length === 0) {
    ...
    const loadingMsg = document.createElement('div');
    loadingMsg.className = 'loading-message';
    loadingMsg.textContent = 'Loading apps from Homebrew...';
...
function renderAppCard(app: App, isInstalled: boolean): HTMLElement {
  ...
  // Simplified safe SVG injection since there is no user input here
  ...
  actionBtn.addEventListener('click', (e) => {
    ...
    const appType = actionBtn.dataset.type;
    if (!appName || !appType) return;

    const isAppInstalled = installedApps.has(appName);
