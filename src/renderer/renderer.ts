...
function renderCategories(): void {
  ...
  CATEGORIES.forEach((cat) => {
    ...
    const countText = isInstalled ? ` (${installedApps.size})` : '';
    btn.textContent = `${cat}${countText}`;
...
function filterApps(): void {
  ...
  requestAnimationFrame(() => {
    // Move redundant lowercase computation outside the loop
    ...
    filteredApps = allApps.filter((app) => {
      // 1. Fastest check: strict equality on type
      if (selectedType !== 'All' && app.type !== selectedType) {
        return false;
      }

      // 2. Fast check: Set lookup or cached string property
      if (selectedCategory === 'Installed') {
        if (!installedApps.has(app.name)) {
          return false;
        }
      } else if (selectedCategory !== 'All') {
        if (getCategoryForApp(app) !== selectedCategory) {
          return false;
        }
      }

      // 3. Slower check: String includes, avoiding IIFE and caching lowercase strings
      if (searchLower) {
        if (!app._nameLower) app._nameLower = (app.name || '').toLowerCase();
        if (!app._descLower) app._descLower = (app.description || '').toLowerCase();
        ...
        if (
          ...
          !app._homeLower.includes(searchLower)
        ) {
          return false;
        }
      }
...
function runCommand(command: string): void {
  ...
  terminalOutput.append(promptSpan, document.createTextNode(` ${command}\n`));
  terminalOutput.scrollTop = terminalOutput.scrollHeight;
