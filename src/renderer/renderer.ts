...
function renderCategories(): void {
  ...
  CATEGORIES.forEach((cat) => {
    ...
    button.textContent = `${cat}${isInstalled ? ' (' + installedApps.size + ')' : ''}`;
...
function filterApps(): void {
  ...
  requestAnimationFrame(() => {
    ...
    // Hoist lowercase operation outside loop
    const searchLower = (searchTerm || '').toLowerCase();
    const hasSearch = searchLower.length > 0;

    filteredApps = allApps.filter((app) => {
      // 1. Order early returns by speed: type check first (direct property check)
      if (selectedType !== 'All' && app.type !== selectedType) {
        return false;
      }

      // 2. Category check (set lookup or cached property check)
      ...
      // 3. Search text check (string includes)
      if (hasSearch) {
        ...
        if (!app._homepageLower) app._homepageLower = (app.homepage || '').toLowerCase();
        if (app._homepageLower.includes(searchLower)) return true;
