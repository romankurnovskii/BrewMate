...
function renderCategories(): void {
  ...
  CATEGORIES.forEach((cat) => {
    ...
    btn.className =
      `category-chip ${isInstalled ? 'installed-category' : ''} ${isActive ? 'active' : ''}`.trim();
...
function getCategoryForApp(app: App): string {
  // ⚡ Bolt Optimization:
  // Cache the category calculation and lowercased fields onto the object directly.
  // This memoization prevents O(N * M) string manipulation and lookups
  // during repeated keystroke filtering.
  if (app._category !== undefined) return app._category;

  if (app._descLower === undefined) app._descLower = (app.description || '').toLowerCase();
  if (app._nameLower === undefined) app._nameLower = (app.name || '').toLowerCase();
...
function filterApps(): void {
  ...
  requestAnimationFrame(() => {
    ...
    // ⚡ Bolt Optimization:
    // Move invariant transformations outside the loop to prevent repeated computation.
    // Time complexity for filtering drops significantly.
