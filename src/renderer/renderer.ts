...
interface App {
  ...
  version: string;
  type: 'cask' | 'formula';
  // Optimization: internal caching for filtering
  _category?: string;
  _nameLower?: string;
  _descLower?: string;
  _homeLower?: string;
...
function getCategoryForApp(app: App): string {
  ...
  const desc = app._descLower;
  const name = app._nameLower;
...
function filterApps(): void {
  ...
  requestAnimationFrame(() => {
    ...
    // Optimization: Cache search string lowercasing outside the loop
    const searchLower = (searchTerm || '').toLowerCase();

    filteredApps = allApps.filter((app) => {
      // 1. Fastest check: Type
      if (selectedType !== 'All' && app.type !== selectedType) {
        return false;
      }

      // 2. Medium check: Category / Installed status
      ...
      // 3. Slowest check: String matching
      if (searchLower) {
        ...
        if (app._homeLower === undefined) app._homeLower = (app.homepage || '').toLowerCase();
        if (app._homeLower.includes(searchLower)) return true;

        return false; // Did not match any search fields
      }
