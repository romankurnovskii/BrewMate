...
interface App {
  ...
  version: string;
  type: 'cask' | 'formula';
  // Internal cache properties for fast filtering
  _category?: string;
  _nameLower?: string;
  _descLower?: string;
  _homeLower?: string;
...
function getCategoryForApp(app: App): string {
  app._descLower = app._descLower ?? (app.description || '').toLowerCase();
  app._nameLower = app._nameLower ?? (app.name || '').toLowerCase();
  ...
  requestAnimationFrame(() => {
    ...
    filteredApps = allApps.filter((app) => {
      ...
      if (selectedCategory === 'Installed') {
        ...
        app._category = app._category ?? getCategoryForApp(app);
      ...
      app._homeLower = app._homeLower ?? (app.homepage || '').toLowerCase();
      return app._homeLower.includes(searchLower);
    });
