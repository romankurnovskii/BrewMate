...
function filterApps(): void {
  ...
  requestAnimationFrame(() => {
    ...
    // Cache invariant values outside the loop
    ...
    filteredApps = allApps.filter((app) => {
      // 1. Check type first (fastest, direct property access)
      if (hasTypeFilter && app.type !== selectedType) {
        return false;
      }

      // 2. Check category next (slower, may require Set lookup or string operations)
      if (hasCategoryFilter) {
        if (isInstalledCategory) {
          if (!installedApps.has(app.name)) return false;
        } else if (getCategoryForApp(app) !== selectedCategory) {
          return false;
        }
      }

      // 3. Check search last (slowest, multiple string operations)
      if (hasSearch) {
        const name = (app.name || '').toLowerCase();
...
function runCommand(command: string): void {
  ...
  terminalOutput.appendChild(commandTextNode);
  terminalOutput.scrollTop = terminalOutput.scrollHeight;
