...
function renderCategories(): void {
  ...
  CATEGORIES.forEach((cat) => {
    ...
    btn.className = `category-chip ${
      isInstalled ? 'installed-category' : ''
    } ${isActive ? 'active' : ''}`;
...
function filterApps(): void {
  ...
  requestAnimationFrame(() => {
    ...
    // ⚡ Bolt: Move invariant string transformation outside the filter loop
    // This avoids calling .toLowerCase() on the searchTerm for every item in allApps (O(N) operation)
