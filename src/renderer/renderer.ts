...
function filterApps(): void {
  ...
  requestAnimationFrame(() => {
    ...
    // Pre-calculate search term outside the loop to avoid O(N) string allocations
    ...
    filteredApps = allApps.filter((app) => {
      ...
      // Early return on category mismatch
      ...
      // Lazy initialization of lowercase strings for search, caching them on the app object
      // This reduces filtering from O(N) string creations per keystroke to O(1) post-warmup
      if (app._nameLower === undefined) {
        app._nameLower = (app.name || '').toLowerCase();
      }
      if (app._descLower === undefined) {
