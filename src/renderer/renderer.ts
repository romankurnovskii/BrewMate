...
interface App {
  ...
  version: string;
  type: 'cask' | 'formula';
  // Cache properties for performance optimization (prevent repeated allocations)
  _category?: string;
  _nameLower?: string;
  _descLower?: string;
...
function renderCategories(): void {
  categoryChips.replaceChildren(); // clear existing chips safely
...
function filterApps(): void {
  ...
  requestAnimationFrame(() => {
    ...
    // Performance optimization: prevent redundant lowering and repeated allocations.
    // Evaluating string combinations and allocations across ~100k items takes a large toll.
    const searchLower = searchTerm ? searchTerm.toLowerCase() : '';

    filteredApps = allApps.filter((app) => {
      // Fast path: type match
      if (selectedType !== 'All' && app.type !== selectedType) {
        return false;
      }

      // Medium path: category match
