...
## 2024-11-09 - Optimized array iteration for 100k items
**Learning:** When iterating over massive arrays (~100k items) in V8, native `for` loops (e.g., `for (let i = 0; i < len; i++)`) consistently outperform `.filter()` and `for...of` loops by avoiding callback closure and iterator protocol overhead.
**Action:** Always prefer native `for` loops over array methods with callbacks or `for...of` loops in the critical rendering or filtering paths for extremely large datasets.
