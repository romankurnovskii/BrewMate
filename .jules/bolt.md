...
## 2024-05-24 - Optimized rendering loops with O(1) Color Maps
**Learning:** Using `Object.values().find()` inside a loop for rendering UI elements reallocates arrays and incurs O(N) lookup costs, causing unnecessary allocations and blocking the UI thread during rendering.
**Action:** Precompute and maintain a `Map` (e.g., `categoryColorMap`) during data ingestion to enable O(1) lookups and eliminate redundant allocations during rendering loops.
